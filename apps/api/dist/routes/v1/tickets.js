...
function roleHasTicketsReadAll(role) {
    if (!role)
        return false;
...
async function broadcastStatusUpdate(channelId, statusId, statusName) {
...
export async function registerTicketsRoutes(app) {
    ...
    app.post('/v1/tickets/assign', async (req, reply) => {
        ...
        try {
            const { ticketID, assignmentID } = bodySchema.parse((req.body ?? {}));
            const ticket = await prisma.ticket.findUnique({ where: { id: ticketID } });
            if (!ticket)
                return reply.status(400).send('Ticket not found');
            ...
            await broadcastStatusUpdate(ticket.channelId, status.id, status.name);
    ...
    app.post('/v1/tickets/close', async (req, reply) => {
        ...
        try {
            ...
            if (!canManage)
                return reply.status(403).send('forbidden');
            const { channelID, discordUserID } = bodySchema.parse((req.body ?? {}));
    ...
    app.post('/v1/tickets/transmission', async (req, reply) => {
        ...
        try {
            const { channelID, problematic, observations, info } = bodySchema.parse((req.body ?? {}));
            const ticket = await prisma.ticket.findUnique({ where: { channelId: channelID } });
            if (!ticket)
                return reply.status(400).send({ success: false, error: 'No ticket found' });
    ...
    app.post('/v1/tickets/create', async (req, reply) => {
        ...
        try {
            const { discordUserID, token, source, metadata } = bodySchema.parse((req.body ?? {}));
            // Validate API key
            ...
            if (hasTicket)
                return reply.status(401).send({ success: false, error: 'User already has an open ticket' });
    ...
    app.post('/v1/tickets/findBy/channelID', async (req, reply) => {
        ...
        try {
            const { channelID } = bodySchema.parse((req.body ?? {}));
            const ticket = await prisma.ticket.findUnique({ where: { channelId: channelID } });
            if (!ticket)
