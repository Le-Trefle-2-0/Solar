...
export async function registerEventsRoutes(app) {
    ...
    app.get('/v1/events/getAvailable', async (req, reply) => {
        try {
            const { channelID, lists } = req.query;
            ...
            if (lists === "ineligible") {
                ...
                const textIds = new Set(textTickets.map(t => t.assignedUserId).filter((id) => !!id));
                ...
                const voiceIds = new Set(voiceTickets.map(t => t.assignedUserId).filter((id) => !!id));
            ...
            const excludedUserIds = new Set(recentTickets
