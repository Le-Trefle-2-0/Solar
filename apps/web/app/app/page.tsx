...
export default async function Home() {
    ...
    return (
        <Page title="Tableau de bord" description="Bienvenue sur votre espace de gestion" className="max-w-4xl mx-auto">
            <div className="flex flex-col gap-6 w-full">
                                    ...
                                    {registeredEvents.map((ev) => (
                                        <li key={ev.id} className="flex items-start justify-between">
                                            <div>
                                                ...
                                                <div
                                                    className="text-sm text-muted-foreground">{format(new Date(ev.start), 'dd/MM/yyyy HH:mm')}</div>
                                            </div>
                                            <Link href={`/app/planning?event=${ev.id}`}
                                                  className="text-sm text-primary">Voir</Link>
                                        </li>
                                    ))}
                                </ul>
                            )}
                        ...
                        <CardContent>
                            {assignedTicket ? (
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div
                                        className="text-sm text-muted-foreground">Statut: {assignedTicket.statusName}</div>
                                    <div className="text-sm text-muted-foreground">Source: {assignedTicket.source}</div>
                                    <div className="pt-2">
                                        <Link href={`/app/ticket/${assignedTicket.id}`}
                                              className="text-sm text-primary">Ouvrir le ticket</Link>
                                    </div>
                                </div>
                            ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <ul className="flex flex-col gap-3">
                                    {eventsNeedingVolunteers.map((ev) => (
                                        <li key={ev.id} className="flex flex-col">
                                            ...
                                            <div className="mt-2">
                                                <Link href={`/app/recruitments?event=${ev.id}`}
                                                      className="text-sm text-primary">S'inscrire</Link>
                                            </div>
                                        </li>
                                    ))}
                                </ul>
                    ...
                    <Card>
                        ...
                        <CardFooter>
                            <Link href="/app/admin/history" className="text-sm text-primary">Voir toutes les
                                notes</Link>
                        </CardFooter>
                    </Card>
                </div>
