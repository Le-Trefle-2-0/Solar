export async function broadcast(room: string | null, event: string, data: any) {
    try {
        ...
        console.log(`[web] Broadcasting event "${event}" to room "${room}"`);
        ...
        if (!response.ok) {
            console.warn(`[web] Broadcast failed with status ${response.status}: ${await response.text()}`);
        } else {
            console.log(`[web] Broadcast of "${event}" successful`);
        }
    } catch (err) {
        console.warn('[web] Failed to broadcast:', err);
    }
}
